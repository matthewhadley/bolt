...
setup () {
  ...
  curl -sf https://raw.githubusercontent.com/matthewhadley/bolt/master/VERSION > $BATS_TMPDIR/fetch/VERSION
}
...
@test "fetch status: returns FAILED when cannot curl source" {
  run fn status $BATS_TMPDIR/fetch/foo https://raw.githubusercontent.com/matthewhadley/bolt/master/missing
  [ $status -eq $STATUS_FAILED ]
}

@test "fetch status: returns FAILED_PRECONDITION when required owner is missing" {
  run fn status $BATS_TMPDIR/fetch/foo https://raw.githubusercontent.com/matthewhadley/bolt/master/VERSION --owner missing
...
@test "fetch status: returns OUTDATED when target file contents differ from source" {
  run fn status $BATS_TMPDIR/fetch/outdated https://raw.githubusercontent.com/matthewhadley/bolt/master/VERSION
  [ $status -eq $STATUS_OUTDATED ]
}

@test "fetch status: returns OK when target file matches source" {
  run fn status $BATS_TMPDIR/fetch/VERSION https://raw.githubusercontent.com/matthewhadley/bolt/master/VERSION
  [ $status -eq $STATUS_OK ]
}

@test "fetch status: returns CONFLICT when target file perms differ from source" {
  run fn status $BATS_TMPDIR/fetch/VERSION https://raw.githubusercontent.com/matthewhadley/bolt/master/VERSION --perms 744
  [ $status -eq $STATUS_CONFLICT ]
}

@test "fetch status: returns CONFLICT when target file owner differs from source" {
  skip_root
  run fn status $BATS_TMPDIR/fetch/VERSION https://raw.githubusercontent.com/matthewhadley/bolt/master/VERSION --owner nobody
  [ $status -eq $STATUS_CONFLICT ]
}

@test "fetch install: creates target file with source contents" {
  run fn install $BATS_TMPDIR/fetch/missing https://raw.githubusercontent.com/matthewhadley/bolt/master/VERSION
  [ $status -eq $STATUS_OK ]
...
@test "fetch install: creates target file and sets perms" {
  run fn install $BATS_TMPDIR/fetch/perms https://raw.githubusercontent.com/matthewhadley/bolt/master/VERSION --perms 664
  [ $status -eq $STATUS_OK ]
